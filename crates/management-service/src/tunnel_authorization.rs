...
impl<O, D> LocalTunnelAuthorizer<O, D> {
    #[must_use]
    pub fn new(oauth2_service: Arc<O>, devices: Arc<D>) -> Self {
        Self {
            oauth2_service,
            devices,
            #[cfg(not(feature = "replica"))]
    ...
    pub fn authorization_provider(
        &self,
        scope: StorageScope,
    ) -> LocalTunnelAuthorizationProvider<O, D> {
...
where
    ...
    C: ClientRepo + Send + Sync + 'static,
    AC: OAuth2AuthorizationCodeRepo + Send + Sync + 'static,
    U: UserRepo + Send + Sync + 'static,
    G: OAuth2UserConsentRepo + Send + Sync + 'static,
    K: KeyRepo + Send + Sync + 'static,
    D: DeviceRepo + Send + Sync + 'static,
{
    async fn authorize(
        &self,
        vault_id: VaultId,
...
pub struct LocalTunnelAuthorizationProvider<O, D> {
    oauth2_service: Arc<O>,
    devices: Arc<D>,
    scope: StorageScope,
}

impl<O, D> Clone for LocalTunnelAuthorizationProvider<O, D> {
    fn clone(&self) -> Self {
        Self {
            oauth2_service: Arc::clone(&self.oauth2_service),
            devices: Arc::clone(&self.devices),
