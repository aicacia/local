<script lang="ts">
    ...
    async function onLoadJwk(key: ManagementKey) {
        ...
        try {
            ...
            jwkKeyId = null;
            jwkError =
                cause instanceof Error ? cause.message : "Failed to load JWK";
                            ...
                            <tbody>
                                {#each keys as key (key.id)}
                                    ...
                                    >
                                        ...
                                        <td class="px-4 py-3"
                                            >{key.derivationPath}</td
                                        >
                                        <td class="px-4 py-3"
                                            >{key.hardened ? "Yes" : "No"}</td
                                        >
                                        <td class="px-4 py-3"
                                            >{key.revokedAt
