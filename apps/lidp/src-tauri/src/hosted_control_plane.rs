...
use lidp_model::contract::{
    ErrorCode, ErrorResponse, Jwks, StorageSession, TrustedDevice, TunnelAuthorization,
    TunnelAuthorizationClaims, TunnelAuthorizationRequest,
};
use lidp_service::{
    oauth2::{decode_jwt, verify_jwt, verify_tunnel_authorization},
    storage_session::StorageScope,
};
use model::contract::{StandardClaims, TokenType, TokenUse};
use reqwest::{Client, Url, redirect::Policy};

use lidp_server::StorageScopeResolver;
...
impl StorageScopeResolver for HostedControlPlane {
    fn resolve(
        &self,
        bearer_token: String,
    ) -> Pin<Box<dyn Future<Output = Result<StorageScope, ErrorResponse>> + Send + '_>> {
        Box::pin(async move {
            self.storage_scope(&bearer_token)
                .await
