...
mod tests {
    ...
    use crate::DeviceEnrollmentService;
    ...
    async fn manages_global_devices() {
        let path =
            std::env::temp_dir().join(format!("idp-device-test-{}.sqlite", std::process::id()));
    ...
    async fn pairing_approval_requires_the_accepting_device_signature() {
        let path = std::env::temp_dir().join(format!(
            "idp-pairing-signature-test-{}.sqlite",
            std::process::id()
        ));
        let _ = std::fs::remove_file(&path);
        let database = Arc::new(Builder::new_local(&path).build().await.unwrap());
    ...
    async fn pairing_request_is_pending_until_approved() {
        let path =
            std::env::temp_dir().join(format!("idp-pairing-test-{}.sqlite", std::process::id()));
        let _ = std::fs::remove_file(&path);
        let database = Arc::new(Builder::new_local(&path).build().await.unwrap());
        idp_model::migrate::up(&database).await.unwrap();
        let repo = LibSqlDeviceRepo::new(Arc::clone(&database));
